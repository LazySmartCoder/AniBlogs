...
def VerifyEmail(request):
    if request.user.is_authenticated:
        sendEmail(request.user.first_name, request.user.username, "OTP | AniBlogs", f"Thank you for undergoing the verification process. Your OTP is {request.user.email}.")
...
def ResendOTP(request):
    if request.user.is_authenticated:
        ...
        userdet.save()
        sendEmail(request.user.first_name, request.user.username, "New OTP | AniBlogs", f"Thank you for requesting an OTP. Your new OTP is {request.user.email}.")
...
def PasswordRecovery(request):
    if request.method == "POST":
        ...
        sendEmail(userdet.first_name, email, "Password Recovery - AniBlogs", f"Thank you for requesting an OTP for password recovery. This happens sometimes that we forgot our passwords of some useful sites like AniBlogs. Your OTP is {forgotpass.OTP}")
...
def PasswordRecovered(request):
    if request.method == "POST":
        ...
        if otp == forgdet.OTP:
            ...
            userdet.save()
            sendEmail(userdet.first_name, userdet.username, "Password Recovered", "This is to inform you that your AniBlogs account password has been recovered. Thank you.")
